...
class MetacityWidget(Gtk.Table):
    ...
    def __init__(self):
        GObject.GObject.__init__(self, rows=13, columns=2)

        c = GConf.Client.get_default()
        c.add_dir('/apps/metacity/global_keybindings', GConf.ClientPreloadType.PRELOAD_NONE)
        c.add_dir('/apps/metacity/keybinding_commands', GConf.ClientPreloadType.PRELOAD_NONE)

        self.attach(Gtk.Label(label=_("Accelerator")), 0,1,0,1)
        self.attach(Gtk.Label(label=_("Action")), 1,2,0,1)
...
class KeyBindingButton(GConfButton):
    ...
    def on_keypress(self, widget, event): # modified from gnome deskbar-applet's DeskbarPreferencesUI.py
        ...
        translation = keymap.translate_keyboard_state(event.hardware_keycode, event.get_state(), event.group)
        if translation == None: # FIXME: metacity can also handle raw keycodes with modifiers (but can compiz?)
            ...
            accel_mods = event.get_state() & Gdk.ModifierType.MODIFIER_MASK & ~(consumed_modifiers | ignored_modifiers)
...
def show_keybinder():
    ...
    buttons.pack_end(close, False, True, 0)

    t = MetacityWidget()

    contents = Gtk.VBox()
    contents.pack_start(t, True, True, 0)
    ...
    l.props.wrap = True
    contents.pack_start(l, True, True, 0)
    contents.pack_end(buttons, False, True, 0)
    d.add(contents)
    d.show_all()
